...
if readelf -d "/usr/local/lib/libwireshark.so" | grep "SONAME" | grep "libwireshark.so.12" ; then
    echo "Found libwireshark.so.12 being used"
else
    echo "Didn't find libwireshark.so.12"
    FindWiresharkLibrary=false
fi

if readelf -d "/usr/local/lib/libwiretap.so" | grep "SONAME" | grep "libwiretap.so.9" ; then
    echo "Found libwiretap.so.9 being used"
else
    echo "Didn't find libwiretap.so.9"
    FindWiresharkLibrary=false
fi

if readelf -d "/usr/local/lib/libwsutil.so" | grep "SONAME" | grep "libwsutil.so.10" ; then
    echo "Found libwsutil.so.10 being used"
else
    echo "Didn't find libwsutil.so.10"
    FindWiresharkLibrary=false
fi
